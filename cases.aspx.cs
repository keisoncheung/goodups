...
public partial class cases : System.Web.UI.Page
{
    ...
    public string Ini()
    {
        ...
        if (nid.Length <= 0) //新闻列表
        {
            ...
            string sql = "select * from (select top " + StartNum + " Id, EditDate,Title,CreateDate,parid,Content,LinkPic,intro,ishot from news " + where + " order by EditDate desc) a where a.Id not in (Select top " + intTemNum + " Id from news " + where + " order by  EditDate desc)";
            if (intTemNum <= 0)
            {
                sql = "select top " + RecordPerPage + " EditDate, Id,Title,CreateDate,parid,Content,LinkPic,intro,ishot from news " + where + " order by EditDate desc";
            }
            ...
            foreach (DataRow dr in dt.Select())
            {
                ...
                webstr += "<dd ><span >" + DateTime.Parse(Convert.ToString(dr["EditDate"])).ToString("yyyy-MM-dd") + " </span> | <a href='cases.aspx?type=" + dr["parid"] + "&nid=" + dr["id"] + "' title='" + dr["title"] + "' target='_blank'> " + title + "</a> </dd>";
                fori++;
            }
            webstr += "</dl>";
            dt.Dispose();
